...
module.exports = class GuildMemberAddListener extends Listener {
    ...
    async exec(member) {
        member = await this.client.users.fetch(member.id).catch(err => console.log(err));
        if(this.client.config.welcomeChannel == 'false' && member.bot) return;
        let welcomeChannel = await this.client.channels.fetch(this.client.config.welcomeChannel).catch(err => console.log(err));
        if(member.bot && welcomeChannel) return welcomeChannel.send(`${member.toString()} left the server, they joined via OAuth.`).catch(err => console.log(err));
        let user = await this.client.invites.findOne({where: {discordUser: member.id, guildID: member.guild.id}});
        if(!user || !user.inviter) return welcomeChannel.send(`${member.tag} left the server but I do not know who invited them.`);
        ...
        if(!inviter) {
            ...
            return welcomeChannel.send(`${member.tag} left the server. They were invited by **${inviter.tag}**`);
        };
        ...
        welcomeChannel.send(`${member.tag} left the server. They were invited by **${inviter.user.tag}** (who has ${inviter.invites - 1} invites).`).catch(err => console.log(err));
